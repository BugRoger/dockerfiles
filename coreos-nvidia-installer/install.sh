...
Type=oneshot
RemainAfterExit=yes
ExecStartPre=/usr/bin/docker pull bugroger/coreos-nvidia-installer:\${VERSION}-$DRIVER_VERSION
ExecStart=/usr/bin/docker run -v /:/rootfs --privileged bugroger/coreos-nvidia-installer:\${VERSION}-$DRIVER_VERSION
